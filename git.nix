...

{
  programs.git = {
    ...
    aliases = {
      ...
      rb =
        "!f() { git rebase -i HEAD^$1; }; f"; # interactive git rebase for the previous X commits
      sq =
      ...
      lg =
        "log --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(blue)<%an>%Creset' --abbrev-commit"; # nice git log with graph
      lf =
        "!f() { git log --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(blue)<%an>%Creset' --abbrev-commit --follow $1; }; f";
